"""Simulated request fulfilment for the `sim-fulfill` skill.
...
``entry: sim-fulfill.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, request: str) -> str:
    """A stable synthetic fulfilment reference — same (run, request) ⇒ same
    ref, so a Temporal activity retry confirms the same fulfilment id."""
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated fulfilment; return the confirmation line."""
    ...
    if not getattr(ctx, "mock", False):
        _record(run_id, {"request": request, "requester": requester})
    return {
        "fulfill_result": (
            f"Fulfilled {request or 'request'} for {requester or 'requester'} (reference {ref})."
        )
    }
