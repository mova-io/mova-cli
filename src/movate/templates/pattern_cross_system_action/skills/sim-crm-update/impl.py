"""Simulated CRM record update for the `sim-crm-update` skill.
...
``entry: sim-crm-update.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, source: str) -> str:
    ...
    return f"CRM-UPD-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated CRM update; return the confirmation line."""
    ...
    return {
        "crm_result": (
            f"Updated the CRM record for: {action_request or 'the requested action'} "
            f"(reference {ref})."
        )
    }
