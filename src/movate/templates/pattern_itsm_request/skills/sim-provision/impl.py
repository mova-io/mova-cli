"""Simulated ITSM provisioning for the `sim-provision` skill.
...
``entry: sim-provision.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, service: str) -> str:
    """A stable synthetic fulfilment reference — same (run, service) ⇒ same
    ref, so a Temporal activity retry confirms the same ticket id."""
    digest = hashlib.sha256(f"{run_id}:{service}".encode()).hexdigest()[:6].upper()
    return f"ITSM-PROV-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated provisioning; return the confirmation line."""
    service = str(input_payload.get("service", ""))
    ...
    if not getattr(ctx, "mock", False):
        _record(run_id, {"service": service, "requester": requester})
    return {
        "provision_result": (
            f"Provisioned {service or 'service'} for {requester or 'requester'} (reference {ref})."
        )
    }
