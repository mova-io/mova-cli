"""Simulated downstream record write for the `sim-record` skill.

The success-path side effect of the external-api-failure scenario: exactly
ONE ``{system: external-api, action: record}`` row per run — regardless of
how many ATTEMPT rows the flaky-call node burned getting there. The pair
(attempt rows = N, record rows = 1) is the scenario's audit story: retries
are visible, the business effect happened once.
...
``entry: sim-record.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, request: str) -> str:
    """A stable synthetic record reference — same (run, request) ⇒ same ref,
    so a Temporal activity retry confirms the same record id."""
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated downstream write; return the confirmation line."""
    request = str(input_payload.get("request", ""))
    provider = str(input_payload.get("provider", ""))
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, request)
    if not getattr(ctx, "mock", False):
        _record(run_id, {"request": request, "provider": provider})
    return {
        "record_result": (
            f"Recorded result for {request or 'request'} from the "
            f"{provider or 'primary'} provider (reference {ref})."
        )
    }
