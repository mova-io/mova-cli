"""Simulated test stage for the `sim-run-tests` skill.
...
``entry: sim-run-tests.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, change: str) -> str:
    """A stable synthetic CI reference — same (run, change) ⇒ same ref, so a
    Temporal activity retry confirms the same test-stage record."""
    digest = hashlib.sha256(f"{run_id}:{change}".encode()).hexdigest()[:6].upper()
    return f"CI-TEST-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated test stage; return the confirmation line."""
    ...
    return {
        "test_result": (
            f"Test stage for '{change}' passed ({ref}): unit + integration suites green "
            "(simulated)."
        )
    }
