"""Simulated mailbox provisioning for the `sim-provision-email` skill.
...
``entry: sim-provision-email.impl:run`` in skill.yaml resolves this file via a
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated mailbox provisioning; return the confirmation line."""
    ...
    if not getattr(ctx, "mock", False):
        _record(run_id, {"employee": employee})
    return {
        "email_result": (
            f"Provisioned the mailbox for {employee or 'the new hire'} (reference {ref})."
        )
    }
