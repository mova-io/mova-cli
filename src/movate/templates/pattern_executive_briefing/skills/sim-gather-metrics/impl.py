"""Simulated metrics source for the `sim-gather-metrics` skill.
...
``entry: sim-gather-metrics.impl:run`` in skill.yaml resolves this file via a
...
input omits it, ADR 100). NEVER does network IO.
"""
...
# Canned, replay-identical business metrics keyed by the `profile` knob. The
# steady profile stays inside every digest risk rule (success_rate ≥ 0.95,
# cost within budget — the digest must count ZERO risk flags); the degraded
# profile breaches BOTH the success-rate floor and the cost budget (the
# digest must flag each breach).
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated metrics pull; return the canned snapshot.
