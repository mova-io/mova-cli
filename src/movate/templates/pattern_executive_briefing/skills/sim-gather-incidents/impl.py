"""Simulated incident source for the `sim-gather-incidents` skill.
...
``entry: sim-gather-incidents.impl:run`` in skill.yaml resolves this file via
...
The data itself is canned + replay-identical, keyed by the ``profile`` knob
(``steady`` | ``degraded``; missing → ``steady`` — the cron-fed production
input omits it, ADR 100). The steady list is EMPTY on purpose: the digest's
risk rules must count zero flags on the clean path. NEVER does network IO.
"""
...
# Canned, replay-identical open-incident lists keyed by the `profile` knob.
# steady = no open incidents (the clean/archive path); degraded = two open
# incidents the digest must flag (one risk flag each).
    ...
    "degraded": [
        {
            ...
            "severity": "sev2",
            "status": "open",
            "summary": "Temporal worker queue backlog above 30 minutes",
        },
        {
            "id": "INC-4105",
            "severity": "sev3",
            "status": "open",
            "summary": "Langfuse trace export lagging by 2 hours",
        },
    ],
}
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated incident pull; return the canned list.

    A missing ``profile`` falls back to ``steady`` (the cron-fed production
    input carries only ``period``); an unknown explicit profile is rejected
    by the input schema upstream — the ``KeyError`` here is the fail-loud
    backstop, never a silent default bucket.
    """
    period = str(input_payload.get("period") or "daily")
    profile = str(input_payload.get("profile") or "steady")
    incidents = _PROFILES[profile]
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    if not getattr(ctx, "mock", False):
        _record(run_id, {"period": period, "profile": profile, "open_count": len(incidents)})
    return {"incidents": [dict(row) for row in incidents]}
