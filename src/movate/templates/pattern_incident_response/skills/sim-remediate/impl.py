"""Simulated ops remediation for the `sim-remediate` skill.
...
def _reference(run_id: str, source: str) -> str:
    """A stable synthetic reference — same (run, source) ⇒ same ref, so a
    Temporal activity retry confirms the same reference."""
    digest = hashlib.sha256(f"{run_id}:{source}".encode()).hexdigest()[:6].upper()
    return f"OPS-REM-{digest}"


# Deterministic failure rule: the simulated ops system can only fix
# software-level faults. An alert whose MESSAGE names one of these markers is
# beyond automated remediation — the attempt is still recorded (and billed a
# ledger row), but the status comes back `failed` so the verify agent (and the
# decision node behind it) route the run to the human escalation gate. The
# rule keys off the alert message — case-input-deterministic — never off LLM
# output, so a certification case can pin the failed path reliably.
_UNREMEDIABLE_MARKERS = ("hardware", "physical", "manual intervention")


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the remediation ATTEMPT; return its deterministic status."""
    alert = input_payload.get("alert") or {}
    service = str(alert.get("service", "")) if isinstance(alert, dict) else ""
    severity = str(alert.get("severity", "")) if isinstance(alert, dict) else ""
    message = str(alert.get("message", "")) if isinstance(alert, dict) else ""
    remediation = str(input_payload.get("remediation", ""))
    failed = any(marker in message.lower() for marker in _UNREMEDIABLE_MARKERS)
    status = "failed" if failed else "applied"
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, service)
    if not getattr(ctx, "mock", False):
        _record(run_id, {"service": service, "severity": severity, "status": status})
    if failed:
        result = (
            f"Remediation attempted on {service or 'the service'} but FAILED: the alert "
            f"indicates a fault automated remediation cannot fix "
            f"(status failed, reference {ref})."
        )
    else:
        result = (
            f"Applied remediation to {service or 'the service'}: {remediation} "
            f"(status applied, reference {ref})."
        )
    return {"remediation_status": status, "remediation_result": result}
