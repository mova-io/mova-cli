"""Simulated outcome recording for the `sim-record-outcome` skill.
...
``entry: sim-record-outcome.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, variant: str) -> str:
    """A stable synthetic outcome reference — same (run, variant) ⇒ same
    ref, so a Temporal activity retry confirms the same outcome record."""
    digest = hashlib.sha256(f"{run_id}:{variant}".encode()).hexdigest()[:6].upper()
    return f"AB-OUT-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the experiment outcome; return the confirmation line."""
    variant = str(input_payload["variant"])
    user_id = str(input_payload.get("user_id", ""))
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, variant)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"user_id": user_id, "variant": variant})
    return {"outcome_result": f"Outcome recorded for variant {variant} ({ref})."}
