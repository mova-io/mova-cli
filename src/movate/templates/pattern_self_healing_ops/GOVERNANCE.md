...
TOOL attempts (`sim-remediate`, then `sim-remediate-retry`), each verified
...
| Triage without authority | the `triage` agent recommends; `sim-remediate`'s outcome is a pure predicate over the FAULT — the LLM cannot talk an attempt into "applied". |
...
| Attempt outcomes are deterministic | `sim-remediate` / `sim-remediate-retry` are pure predicates over the fault (transient faults need the retry; hardware never applies) | the verification decisions replay identically on Temporal. |
...
- Point `sim-remediate` / `sim-remediate-retry` at your real runbook
