...
* HUMAN      → stubbed in Phase 1 (``NotImplementedError`` — see ADR 054 Phase 2:
  ``workflow.wait_condition`` + signal, durable HITL).
...
LINT_HUMAN_NODE_PHASE2 = "TEMPORAL_HUMAN_NODE_PHASE2"
...
class TemporalCompiler:
    ...
    def lint(self, spec: WorkflowGraph) -> list[LintIssue]:
        ...
        * Skills whose ``capabilities.deterministic`` flag is false — the
          ...
          confirm idempotency.
        * HUMAN nodes — stubbed in Phase 1 (Phase 2 promotes to working).
...
def supports_spec(spec: WorkflowSpec) -> bool:
    """Cheap Phase-1 readiness check for ``workflow.yaml: runtime: temporal``.

    True when the spec carries no node types Phase 1 rejects (HUMAN). The
    runner Protocol can route to the native backend with a clear message
    when this returns False.
    """
    return not any(getattr(n, "type", None) == "human" for n in spec.nodes)
