...
class TemporalCompiler:
    ...
    def _emit_workflow_defn(
        ...
        if has_decision:
            # Pure routing helper — imported inside the passed-through block so it
            # is usable in the deterministic workflow sandbox (ADR 094 D3).
            header.insert(
                header.index("    )") + 1,
                "    from movate.core.workflow.decision import evaluate_decision",
            )
