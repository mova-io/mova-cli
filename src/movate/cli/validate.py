...
def _check_marketplace_metadata(spec: AgentSpec) -> None:
    ...
    Lightweight advisory checks — none of these are hard errors:

    1. **no metadata block** — emits a dim discovery hint (exit 0, no color).
       Non-intrusive; purely a discovery prompt for operators who haven't
       heard of the catalog feature yet.

    2. **owner** — if set to an empty string, emits a yellow warning.
       A non-empty string (email or team name) is accepted without further
       validation.

    3. **examples** — if non-empty, each entry should have both ``input`` and
