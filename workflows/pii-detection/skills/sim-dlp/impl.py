"""Simulated DLP quarantine for the `sim-dlp` skill.
...
``entry: sim-dlp.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, source: str) -> str:
    """A stable synthetic quarantine reference — same (run, source) ⇒ same
    ref, so a Temporal activity retry confirms the same quarantine id."""
    digest = hashlib.sha256(f"{run_id}:{source}".encode()).hexdigest()[:6].upper()
    return f"DLP-QTN-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated quarantine; return the confirmation line."""
    source = str(input_payload.get("source", ""))
    pii_count = int(input_payload.get("pii_count", 0) or 0)
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, source)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"source": source, "pii_count": pii_count})
    return {
        "dlp_result": (
            f"Quarantined document from {source or 'unknown source'} "
            f"with {pii_count} PII value(s) masked (reference {ref})."
        )
    }
