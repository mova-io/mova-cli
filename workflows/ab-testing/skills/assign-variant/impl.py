"""Deterministic variant assignment for the `assign-variant` skill.
...
``entry: assign-variant.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _variant(user_id: str) -> str:
    """The deterministic 2-way split: parity of the user_id's SHA-256.

    Python's builtin ``hash()`` is salted per process (PYTHONHASHSEED) and
    MUST NOT be used here — it would assign the same user different variants
    on different workers and break Temporal replay determinism. SHA-256 is
    process- and platform-independent: same user_id ⇒ same variant, forever.
    """
    return "a" if int(hashlib.sha256(user_id.encode()).hexdigest(), 16) % 2 == 0 else "b"


def _reference(run_id: str, user_id: str) -> str:
    """A stable synthetic assignment reference — same (run, user) ⇒ same
    ref, so a Temporal activity retry confirms the same assignment."""
    digest = hashlib.sha256(f"{run_id}:{user_id}".encode()).hexdigest()[:6].upper()
    return f"AB-ASSIGN-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Assign the variant deterministically; record the assignment row."""
    user_id = str(input_payload["user_id"])
    variant = _variant(user_id)
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, user_id)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"user_id": user_id, "variant": variant})
    return {
        "variant": variant,
        "assign_result": (f"User {user_id} deterministically assigned variant {variant} ({ref})."),
    }
