"""Simulated eval run for the `eval-runner` skill.
...
``entry: eval-runner.impl:run`` in skill.yaml resolves this file via a PEP 420
...
# Calibrated fixture table — the ONLY source of scores. Deterministic per
# fixture so the certification gates are stable run after run; an unknown
# fixture raises KeyError (the input schema's enum is the first line of
# defense, this is the fail-loud backstop — no silent default score).
_FIXTURES: dict[str, dict[str, float]] = {
    "passing": {"accuracy": 0.94, "safety": 0.97, "overall": 0.93},
    "failing": {"accuracy": 0.45, "safety": 0.62, "overall": 0.41},
}


def _reference(run_id: str, fixture: str) -> str:
    """A stable synthetic eval-run reference — same (run, fixture) ⇒ same
    ref, so a Temporal activity retry confirms the same eval run id."""
    digest = hashlib.sha256(f"{run_id}:{fixture}".encode()).hexdigest()[:6].upper()
    return f"EVAL-RUN-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated eval run; return the overall score + report."""
    candidate = str(input_payload.get("candidate", ""))
    version = str(input_payload.get("version", ""))
    fixture = str(input_payload["fixture"])
    scores = _FIXTURES[fixture]
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, fixture)
    if not getattr(ctx, "mock", False):
        _record(
            run_id,
            _ACTION,
            {"candidate": candidate, "version": version, "fixture": fixture, "scores": scores},
        )
    metric_text = ", ".join(f"{k}={v:.2f}" for k, v in sorted(scores.items()))
    return {
        "eval_score": scores["overall"],
        "eval_report": (
            f"Eval run {ref} for {candidate or 'candidate'} {version or '(unversioned)'}: "
            f"{metric_text}."
        ),
    }
