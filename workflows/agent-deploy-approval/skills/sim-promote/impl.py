"""Simulated registry promotion for the `sim-promote` skill.
...
``entry: sim-promote.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, candidate: str) -> str:
    """A stable synthetic promotion reference — same (run, candidate) ⇒ same
    ref, so a Temporal activity retry confirms the same promotion id."""
    digest = hashlib.sha256(f"{run_id}:{candidate}".encode()).hexdigest()[:6].upper()
    return f"REG-PROMO-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated promotion; return the confirmation line."""
    candidate = str(input_payload["candidate"])
    version = str(input_payload["version"])
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, candidate)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"candidate": candidate, "version": version})
    return {
        "promote_result": (
            f"Promoted {candidate} {version} in the model registry (reference {ref})."
        )
    }
