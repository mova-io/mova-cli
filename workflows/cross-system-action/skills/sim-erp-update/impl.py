"""Simulated ERP vendor update for the `sim-erp-update` skill.
...
``entry: sim-erp-update.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, source: str) -> str:
    ...
    return f"SAP-VND-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated ERP vendor update; return the confirmation line."""
    ...
    return {
        "erp_result": (
            f"Updated the ERP vendor master for: {action_request or 'the requested action'} "
            f"(reference {ref})."
        )
    }
