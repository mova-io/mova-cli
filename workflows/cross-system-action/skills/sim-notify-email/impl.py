"""Simulated notification email for the `sim-notify-email` skill.
...
``entry: sim-notify-email.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, source: str) -> str:
    ...
    return f"EMAIL-SND-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated notification send; return the confirmation line."""
    action_request = str(input_payload.get("action_request", ""))
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, action_request)
    if not getattr(ctx, "mock", False):
        _record(run_id, {"action_request": action_request})
    return {
        "email_result": (
            f"Sent the completion notification for: "
            f"{action_request or 'the requested action'} (reference {ref})."
        )
    }
