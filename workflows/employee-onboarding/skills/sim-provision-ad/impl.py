"""Simulated Active Directory provisioning for the `sim-provision-ad` skill.
...
``entry: sim-provision-ad.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, source: str) -> str:
    ...
    return f"AD-ACCT-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated AD provisioning; return the confirmation line."""
    ...
    if not getattr(ctx, "mock", False):
        _record(run_id, {"employee": employee, "role": role})
    return {
        "ad_result": (
            f"Provisioned the Active Directory account for {employee or 'the new hire'} "
