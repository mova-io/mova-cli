"""Simulated pipeline step for the `sim-step` skill.

One PARAMETERIZED step skill serves both reliable pipeline stages: the
workflow's `input:` literal map (ADR 097 D1) names the step (`step1` /
`step3`), and the recorded ledger action IS that step name — so the
certification driver can pin "step1 happened exactly once" independently of
the flaky step's retries. The recovery proof reads: step1 rows = 1 while
step2_attempt rows = 2 ⇒ Temporal replayed only the failed activity; this
completed step's recorded effect was never re-executed.
...
``entry: sim-step.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, step: str) -> str:
    """A stable synthetic step reference — same (run, step) ⇒ same ref, so a
    Temporal activity retry confirms the same step id."""
    digest = hashlib.sha256(f"{run_id}:{step}".encode()).hexdigest()[:6].upper()
    return f"STEP-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated step; return the completion line."""
    step = str(input_payload.get("step", ""))
    request = str(input_payload.get("request", ""))
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, step)
    if not getattr(ctx, "mock", False):
        _record(run_id, step or "step", {"request": request})
    return {
        "step_result": (f"Completed {step or 'step'} for {request or 'request'} (reference {ref}).")
    }
