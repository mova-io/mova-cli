"""Simulated score recording for the `sim-record-score` skill.
...
``entry: sim-record-score.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    ...
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"score": score})
    return {"score_result": f"Sampled score {score:.2f} recorded ({ref})."}
