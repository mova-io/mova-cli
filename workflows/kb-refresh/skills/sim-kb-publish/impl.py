"""Simulated KB publication for the `sim-kb-publish` skill.

NAMED ``sim-kb-publish`` (not ``sim-publish``) ON PURPOSE: the
content-publishing scenario already ships a ``sim-publish`` skill, and the
python backend resolves skill entries by MODULE name (``sim-publish.impl``)
with the skill dir's parent on ``sys.path`` — two same-named skills in one
long-lived worker process would collide in ``sys.modules`` and the second
workflow would silently import the first one's impl. A unique name makes the
collision impossible; the LEDGER row keeps the certified shape
...
``entry: sim-kb-publish.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, chunk_count: int) -> str:
    """A stable synthetic publication reference — same (run, chunks) ⇒ same
    ref, so a Temporal activity retry confirms the same publication id."""
    digest = hashlib.sha256(f"{run_id}:{chunk_count}".encode()).hexdigest()[:6].upper()
    return f"KB-PUB-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated publication; return the confirmation line."""
    ingest_result = input_payload.get("ingest_result")
    ...
    ref = _reference(run_id, chunk_count)
    if not getattr(ctx, "mock", False):
        _record(run_id, {"chunk_count": chunk_count})
    return {
        "publish_result": (
            f"Published {chunk_count} chunk(s) to the knowledge base (reference {ref})."
        )
    }
