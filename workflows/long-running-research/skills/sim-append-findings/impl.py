"""Simulated research-log append for the `sim-append-findings` skill.

The accumulating side effect of the long-running-research scenario: each
scheduled increment appends one ``{system: research, action: append}`` row
whose payload carries the increment number — so the ledger, read by
``run_id`` per increment (or across the series), IS the research log the
cron schedule builds up fire by fire (ADR 100 D1).
...
``entry: sim-append-findings.impl:run`` in skill.yaml resolves this file via
...
def _reference(run_id: str, topic: str, increment: int) -> str:
    """A stable synthetic log reference — same (run, topic, increment) ⇒ same
    ref, so a Temporal activity retry confirms the same append id."""
    digest = hashlib.sha256(f"{run_id}:{topic}:{increment}".encode()).hexdigest()[:6].upper()
    return f"LOG-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated research-log append; return the confirmation."""
    topic = str(input_payload.get("topic", ""))
    increment = int(input_payload.get("increment", 0) or 0)
    findings = str(input_payload.get("findings", ""))
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, topic, increment)
    if not getattr(ctx, "mock", False):
        # The increment rides the payload — the ledger reads as the series'
        # research log. Findings are excerpted: the ledger is an audit trail,
        # not a document store.
        _record(run_id, {"topic": topic, "increment": increment, "findings": findings[:200]})
    return {
        "append_result": (
            f"Appended increment {increment} findings for {topic or 'topic'} "
            f"to the research log (reference {ref})."
        )
    }
