"""Simulated agent quality health check for the `sim-health-check` skill.
...
The quality metrics are CANNED + CLOSED: a fixed catalog keyed off
``agent_name``, so the score that drives the workflow's routing replays
identically on Temporal. An agent outside the catalog is a loud ``KeyError``
(the sim-audit-store posture) — no silent "probably healthy" default that
would green-wash a typo'd name.

The python skill backend puts this skill dir's PARENT on ``sys.path``
(``movate/core/skill_backend/python.py::_resolve``), so the
``entry: sim-health-check.impl:run`` in skill.yaml resolves this file via a
...
_SYSTEM = "monitor"
_ACTION = "health_check"

# The canned monitor catalog: agent name → (quality_score, symptom). One
# healthy agent (gte the workflow's 0.8 gate), one degraded-but-fixable
# symptom, and one model-drift symptom (the fault class sim-apply-fix
# deterministically CANNOT fix — "drift" in the symptom fails the fix).
_CANNED: dict[str, tuple[float, str]] = {
    "order-tracker": (0.93, "none"),
    "invoice-parser": (
        0.55,
        "elevated output-schema validation failures on recent runs",
    ),
    "support-summarizer": (
        0.41,
        "model drift: hallucinated order numbers rising vs the eval baseline",
    ),
}
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Return the canned quality metrics; record the health-check ledger row.

    Raises ``KeyError`` for an agent outside the canned catalog — fail loud,
    never a silent "healthy" default.
    """
    agent_name = str(input_payload.get("agent_name", ""))
    quality_score, symptom = _CANNED[agent_name]
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    if not getattr(ctx, "mock", False):
        _record(
            run_id,
            {"agent_name": agent_name, "quality_score": quality_score, "symptom": symptom},
        )
    return {"quality_score": quality_score, "symptom": symptom}
