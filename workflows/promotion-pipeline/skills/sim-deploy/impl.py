"""Simulated production promotion for the `sim-deploy` skill.
...
``entry: sim-deploy.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, change: str) -> str:
    """A stable synthetic deploy reference — same (run, change) ⇒ same ref,
    so a Temporal activity retry confirms the same promotion record."""
    digest = hashlib.sha256(f"{run_id}:{change}".encode()).hexdigest()[:6].upper()
    return f"PIPE-PROD-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated production promotion; return the confirmation."""
    ...
    return {"deploy_result": f"Promoted '{change}' to production (reference {ref})."}
