...
- [~] **Resume API surface** `[HIGH] [v1.1] [in-progress: surface shipped, HITL pairing pending]` — [src/movate/core/workflow/resume.py](src/movate/core/workflow/resume.py) ships `resume_workflow(run_id, *, payload, graph, executor, storage, tenant_id) -> WorkflowResult`. Validates the surface that the HTTP endpoint + CLI counterpart will wrap: storage-backed `WorkflowRunRecord` lookup with tenant-aware filtering (cross-tenant returns `ResumeNotFound` identical to missing-id, so existence isn't leaked), checkpointer-required check with operator-facing YAML pointer, payload merge into final_state, async checkpointer context-manager handling. 5 tests in [tests/test_workflow_resume.py](tests/test_workflow_resume.py). **Deferred to the HITL PR**: the actual LangGraph `update_state` + `ainvoke(None, config={thread_id})` body — without HITL nodes there's no organic pause to resume FROM, so the integration tests need HITL to be exercisable end-to-end. The HTTP endpoint and CLI command also land with HITL since they share the same end-to-end test infrastructure.
