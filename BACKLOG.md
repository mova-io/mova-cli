...
**Stack-defense + MDK FAQ docs shipped this session.** Two new operator-facing docs that turn frequent customer-review + team-onboarding questions into linkable reference. `docs/stack-defense.md` (506 lines) gives Deva-grade defensible answers per open-source dep that lands in customer VPCs: license + alternatives considered + customer-VPC implications + removal cost + maturity signals per Pydantic / FastAPI / uvicorn / LiteLLM / httpx / asyncpg / aiosqlite / cryptography / bcrypt / OpenTelemetry / Langfuse / Jinja2 / jsonschema / PyYAML / Anthropic SDK / OpenAI SDK / LangChain core / pgvector / Apache AGE. Includes hard "won't add" license list (GPL/AGPL/SSPL/BSL), new-dep process flow, and a 6-question "for customer review sessions" FAQ. `docs/faq.md` (884 lines) is the team-facing basic/medium/advanced reference: §1 (16 basic questions on what MDK is + why), §2 (16 medium questions on runtime architecture / executor pipeline / tool-use loop / canonical config split / deploy), §3 (advanced questions on codebase organization / extension points / cost-drift / eval reconciliation), §4-§7 (operations, eval+quality, security+multi-tenancy, roadmap+comparison vs LangChain/LlamaIndex/Vellum). **PR #88. Zero code changes; docs-only.**
