...
**v0.5 stage 4 shipped this session — movate is now a runtime, not a queue.** 10 new tests + 1 retired (391 unit + 3 smoke = 394 total). `runtime/dispatch.py` with `WorkerDispatch.execute_job(job) → DispatchOutcome` (pure logic; agent + workflow paths; unknown target / executor crash → terminal ERROR). `runtime/worker.py` with `Worker.run_one_cycle()` (deterministic, used in tests) and `Worker.run_forever(stop_event)` (CLI loop, SIGINT/SIGTERM cancellable). `runtime/registry.scan_workflows(path)` mirrors `scan_agents`. `movate worker` CLI replaces the stub: scans agents+workflows, instantiates the dispatch + loop, registers signal handlers for clean shutdown. `RunResponse` gained `run_id` so the worker can mirror it into `JobRecord.result_run_id`. **Climactic end-to-end binary smoke green**: two real processes (`movate serve` + `movate worker`) — POST /run returned 202 queued → worker claimed and executed → poll showed `status: success`, `result_run_id: 106c31e9-...` (matching the persisted RunRecord), full lifecycle in ~112ms.

1. [ ] **v0.5 stage 5: PostgresProvider port** `[HIGH] [v0.5] [next] [≤2d]` — same conformance suite, switches to `SELECT ... FOR UPDATE SKIP LOCKED` for the claim path.
2. [ ] **Tag v0.5.0** `[MED] [v0.5] [≤30m]` — once Postgres lands; cut the release.
