...
- [x] **HITL nodes** `[—] [v1.1] [done]` — shipped via Tier 2 §HITL nodes below.
...
- [~] **`workflow/compilers/langgraph.py`** `[HIGH] [v1.1] [in-progress: linear + conditional + parallel + HITL AGENT cases shipped]` — alternative compiler from `WorkflowGraph` IR; gated by `workflow.yaml: runtime: langgraph` (default `homegrown`). Linear AGENT, conditional edges, parallel fan-out/fan-in, and HUMAN-node pause/resume all ship and pass end-to-end. Tool nodes / FUNCTION / SUB_WORKFLOW remain rejected via the `can_compile` capability check with an operator-facing reason; each follow-up PR flips one rejection branch. Spike `scripts/langgraph_prototype.py` deleted.
...
- [x] **Resume API + body** `[HIGH] [v1.1] [done]` — [src/movate/core/workflow/resume.py](src/movate/core/workflow/resume.py) ships `resume_workflow(run_id, *, payload, graph, executor, storage, tenant_id) -> WorkflowResult` with the full body wired through `run_via_langgraph(resume_payload=..., resume_as_node=...)`. Surface: storage-backed `WorkflowRunRecord` lookup with tenant-aware filtering (cross-tenant returns `ResumeNotFound` identical to missing-id, no existence leak), checkpointer-required gate, PAUSED-status gate (terminal runs refuse with an actionable hint), payload validated against the paused HUMAN node's `resume_payload_schema` before invoking LangGraph. Body: `aupdate_state(payload, as_node=record.pause_at)` advances past the `interrupt_before` (without `as_node` LangGraph re-pauses indefinitely); `ainvoke(None, config)` continues from the merged checkpoint; the resumed run can itself re-pause if another HUMAN node fires downstream. Exercised end-to-end in [tests/test_workflow_hitl.py](tests/test_workflow_hitl.py). HTTP endpoint + CLI counterpart land in a follow-up PR — they wrap this seam.
- [x] **HITL nodes (`type: human`)** `[HIGH] [v1.1] [done]` — first-class node type; pause graph via LangGraph's `interrupt_before`; resume via the API above. `NodeSpec.type` widens to `Literal["agent", "human"]`; `ref` becomes optional; `resume_payload_schema` required on HUMAN nodes at YAML parse time (Pydantic `model_validator`). `WorkflowStatus.PAUSED` + `WorkflowRunRecord.pause_at` persisted so cross-process resume can pass the correct `as_node`. Compile-time validation: HUMAN + no checkpointer → `WorkflowCompileError` pointing at the YAML field; HUMAN under `runtime: homegrown` → error pointing at `runtime: langgraph` (homegrown has no pause semantics). LangGraph compiler builds `interrupt_before=<human node ids>`, emits no-op runner fns for HUMAN nodes, detects pause via `aget_state()`, and returns `WorkflowResult(status=PAUSED, pause_at=..., resume_payload_schema=...)` for the HTTP wrapper. 7 end-to-end tests in [tests/test_workflow_hitl.py](tests/test_workflow_hitl.py) (3-node `classify (AGENT) → approve (HUMAN) → notify (AGENT)` pause/resume, invalid-payload rejection, cross-tenant 404 even when paused).
