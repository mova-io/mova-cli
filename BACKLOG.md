...
**Project state dir renamed `.movate/` → `.mdk/` — ADR 011.** Read-compat resolver (`project_state_dir`) + opt-in `mdk migrate-state`; machine-global `~/.movate/` (credentials/config) untouched.

**Run-side auth resilience — ADR 012, all three pillars shipped.** (a) one-shot guarded **401 auto-recovery** on `--target` calls (auto-refresh + single retry); (b) optional **OS-keychain credential backend** (`mdk[keychain]`, `keyring`) behind `CredentialsStore`, file stays default; (c) **OIDC** — the runtime accepts an OIDC JWT alongside opaque `mvt_*` keys (PyJWT, asymmetric-only alg allowlist, fail-closed audience), plus an `az`-based client token provider (`azure-identity` deferred pending sign-off). All default-OFF, opaque-key path byte-for-byte unchanged.

**`mdk fleet` — read-only cross-target ops.** Concurrent health + agent-inventory view across all configured targets (`fleet status [--json]`) + `fleet logs`. Lifecycle mutations (restart/scale) deliberately deferred to a future deployment-lifecycle ADR.

**Governance.** Added `CLAUDE.md` (operating rules), `docs/architecture-principles.md` (layer map / boundary contract), `.github/CODEOWNERS`. Version model is CalVer `YYYY.M.D.N` (now `2026.5.23.x`).
...
19. [x] **Teams Slice 3.1.d — file attachment handling** `[HIGH] [v0.7] [done 2026-05-13]` — drag agent.zip / dataset.jsonl into Teams; bot validates + reports back as a card. 33 new tests. PR [#86](https://github.com/mova-io/mova-cli/pull/86). Closes [#68](https://github.com/mova-io/mova-cli/issues/68).
...
27. [x] **ADR 004 — pgvector memory tiers (design)** `[HIGH] [v0.8] [superseded by ADR 009]` — **Superseded by [ADR 009 — pgvector KB storage](docs/adr/009-pgvector-kb-storage.md)** (shipped 2026-05-23), which made + implemented the core decisions this ADR was meant to design: pgvector on the existing Postgres (no separate vector DB), embedding model/dim config (`MOVATE_EMBED_MODEL`/`MOVATE_EMBED_DIM`), the `vector(N)` schema + HNSW index, and eval integration via the faithfulness/coverage dims. The broader "multiple named memory tiers per agent" framing (which tier an agent reads/writes) is not yet built — re-open a narrower item if/when that abstraction is needed. **Issue [#76](https://github.com/mova-io/mova-cli/issues/76).**
...
49. [x] **Graph store** `[MED] [v1.2] [superseded by ADR 010]` — **Superseded by [ADR 010 — GraphRAG behind StorageProvider](docs/adr/010-graphrag.md)** (shipped 2026-05-23). Knowledge-graph entities/relations + bounded k-hop expansion now run as a queryable substrate behind the `StorageProvider` Protocol, multi-backend (SQLite + Postgres) — rather than the originally-scoped **Apache AGE** extension. AGE remains a possible future StorageProvider backend if a customer needs native graph queries, but the memory-tier #2 goal is met without it.
...
- **ADR 005 — Apache AGE knowledge graph + d3.js viz** — flashy demo piece, no customer pull yet. Tracked in [#78](https://github.com/mova-io/mova-cli/issues/78).
- **SaaS onboarding flow** — no customer pull yet. Tracked in [#81](https://github.com/mova-io/mova-cli/issues/81).
- **Worktree hygiene** (`my-sandisk-agent/`, `openai-feedback.md` untracked) — 15-minute idle-time cleanup. Tracked in [#77](https://github.com/mova-io/mova-cli/issues/77).
- **Teams Slice 3.1.e — manifest + Bot Service registration** — gated on Azure tenant migration ([#65](https://github.com/mova-io/mova-cli/issues/65)). Lift when the email response lands.
...
[#65–#69 + #70 + #72](https://github.com/mova-io/mova-cli/issues?q=is%3Aopen+label%3Ateams-integration)
track each slice; status here mirrors the v0.7 milestone.

- [x] **ADR 003 — Teams as a self-serve front door (design)** `[HIGH] [v0.7] [done 2026-05-13]` — design doc with three vertical slices (3.1 run-existing → 3.2 eval-with-upload → 3.3 saved-configs). Status: Proposed (open questions on multi-tenant prospects, Langfuse trace linking across tenants, mobile UX, streaming). PR [#60](https://github.com/mova-io/mova-cli/pull/60).
- [x] **Teams 3.1.a — Bot Framework webhook skeleton** `[HIGH] [v0.7] [done 2026-05-13]` — hand-rolled Activity protocol + `mdk teams-bot serve` Typer command + 28 tests. Hand-rolled instead of `botbuilder-core` to avoid ~30MB of transitive deps; SDK can land in the hardening PR. PR [#63](https://github.com/mova-io/mova-cli/pull/63).
- [x] **Teams 3.1.b — Adaptive Cards + MovateClient integration** `[HIGH] [v0.7] [done 2026-05-13]` — `@movate run faq-agent {...}` actually executes and renders a card with response/cost/latency/optional trace link. Five outcome variants → four card templates. PR [#82](https://github.com/mova-io/mova-cli/pull/82). Closes [#66](https://github.com/mova-io/mova-cli/issues/66).
...
- [x] **Teams 3.1.d — file attachment handling** `[HIGH] [v0.7] [done 2026-05-13]` — drag agent.zip / dataset.jsonl into Teams; validate + ingest before run. PR [#86](https://github.com/mova-io/mova-cli/pull/86). Closes [#68](https://github.com/mova-io/mova-cli/issues/68).
...
- [ ] **Teams hardening — JWT validation** `[MED] [v1.0] [1w]` — production auth before any public Teams exposure. Issue [#70](https://github.com/mova-io/mova-cli/issues/70).
- [ ] **Teams 3.2 — eval with bring-your-own data** `[HIGH] [v0.8] [1w]` — drag dataset + agent, scorecard updates per case via Bot Framework `UpdateActivity`. Surfaces the 4-dim rollup that shipped on 2026-05-13. Issue [#72](https://github.com/mova-io/mova-cli/issues/72).
