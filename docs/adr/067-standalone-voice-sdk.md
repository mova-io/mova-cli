...
**Status:** Accepted (2026-06-02) — see "Consolidation update" at the end of this ADR; D1 (separate repo + PyPI) revised, the rest of the decision stands as written.
**Date:** 2026-06-01
...
D1 (separate `movate-voice` repo + PyPI distribution) is **revised**. After
shipping the standalone scaffold to v0.1.0 (the frozen
[`mdk-voice`](https://github.com/Movate/mdk-voice) snapshot) and running the
demo end-to-end, dev moved back **into `movate-cli`** as a sub-package
(`src/movate/voice/`) and is shipped via the existing `mdk[voice]` extra
rather than a separate PyPI dist. **Reason:** the demo and the voice runtime
are co-evolving daily; two-repo sync cost (PR-per-feature, bump in mdk for
every voice change) was the dominant friction in the first two days. The
v0.1.0 SDK sdist + wheel remain as a tagged reference point in the original
repo for any consumer that needs a pure-voice install.

**Everything else in this ADR stands as written.** The architectural seam
(D2 `AgentTurn` Protocol, D3 the pipeline takes `agent=` not `executor=`, D4
mdk wraps the Executor in `ExecutorAgentTurn`) is implemented exactly as
specified — see [`src/movate/voice/agent_turn.py`](../../src/movate/voice/agent_turn.py),
[`src/movate/voice/pipeline.py`](../../src/movate/voice/pipeline.py), and
[`src/movate/runtime/voice_agent.py`](../../src/movate/runtime/voice_agent.py)
(the only mdk-side place that knows about `RunRequest` / `AgentBundle` on the
voice path). The seam is intact: an `AgentTurn` impl outside this repo (Lyzr,
LangGraph, a bare async fn) still works against the same pipeline. If a future
consumer needs a pure-voice install with no mdk, the *re*-extraction is a
packaging step against the same code, not another refactor — the seam already
draws the line.

**ADR 068 / ADR 069** are likewise consolidated into this repo with the same
"seam stands, packaging revised" framing — see their consolidation updates.
