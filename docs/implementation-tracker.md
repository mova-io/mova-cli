...
| 31 | **Stale-job reaper / visibility timeout** — requeue jobs orphaned in `running` when a worker hard-crashes (OOM/SIGKILL/node loss); dead-letter once the retry budget is exhausted. Reaper runs in the worker loop (primary) + scheduler tick (scaled-to-zero backstop). No schema change (reuses `claimed_at`/`attempt_count`). | A | 017 / feature | M (~30m) | scheduler, worker | ✅ | #410 | `2026.5.24.20` |
| 32 | **Distributed trace propagation across the queue** — inject W3C `traceparent` into `JobRecord` at enqueue, extract + continue in the worker so submit→queue→execute is ONE trace in App Insights (additive `trace_context` column; ADR 019) | B | 015 / 019 | M–L (~45m) | #62 | ✅ | #411 | `2026.5.24.21` |
| 33 | **OTel metrics instruments** — `Meter` + counters/histograms (queue depth, job latency, dead-letter rate, in-flight, per-tenant tokens/cost) feeding the item-27 alert rules | B | 015 | M (~35m) | #62 | ✅ | #412 | `2026.5.24.22` |
| 34 | **Per-job execution timeout** — bound a hung provider call (→ retryable timeout error → retry/dead-letter); kept below the #31 reaper visibility window so a hung job resolves before the reaper would double-run it. _(Run cancellation split to #36.)_ | A | 017 / feature | M (~30m) | #31 | ✅ | #413 | `2026.5.24.23` |
| 35 | **Control-plane audit telemetry** — structured audit events for key mint/revoke/rotate, canary promote/rollback (the "who did what, when" trail); structured `movate.audit` log (→ Log Analytics) + best-effort span event (→ App Insights). _(Drift auto-rollback audit deferred to #40.)_ | B | 013 / 016 | M (~30m) | — | ✅ | #414 | `2026.5.24.24` |
| 36 | **Run cancellation** — cancel a queued/in-flight async run (queued: mark CANCELLED, claim skips it; in-flight: cooperative cancel-flag check before writing the terminal status). New `JobStatus.CANCELLED` + API endpoint (admin/run scope) + CLI. Split from #34. | A | 017 / feature | M (~30m) | #31, #34 | ⬜ | | `____` |
| 37 | **Submission idempotency** — `Idempotency-Key` on run/job submit → dedupe store, so a client retry doesn't double-enqueue (complements trigger idempotency #23 and bounds the reaper/timeout at-least-once story). | A | feature | M (~30m) | — | ⬜ | | `____` |
| 38 | **Log↔trace correlation** — a `logging` filter that stamps the active `trace_id`/`span_id` onto every record so App Insights links each log line to its distributed trace (makes #32/#33 navigable end-to-end). | B | 015 | S (~20m) | #62 | ⬜ | | `____` |
| 39 | **Migration concurrency lock** — advisory lock around `_run_migrations` so concurrent pod startup on a fresh DB can't race the idempotent `ALTER`s (verify-then-harden). | B | feature | S (~20m) | — | ⬜ | | `____` |
| 40 | **Drift auto-rollback audit event** — the deferred #35 one-liner in `dispatch._maybe_auto_rollback` (`action="canary.auto_rollback"`), kept off-limits during the parallel #34 build. | B | 016 | S (~10m) | #35 | ⬜ | | `____` |
| 41 | **Fix: App Insights connectionString empty at CAE preflight** (live-Azure finding) — the CAE `openTelemetryConfiguration` references App Insights' runtime `ConnectionString`, which ARM **preflight** evaluates as empty (`az bicep build` passes; `az deployment group create` fails preflight). Restructure so it resolves post-creation; verify with `az deployment group validate`. | A | 015 | M (~30m) 🔒 | #62 | 🔄 | | `____` |
