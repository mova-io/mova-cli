"""Simulated equipment ordering for the `sim-provision-equipment` skill.
...
``entry: sim-provision-equipment.impl:run`` in skill.yaml resolves this file via a
...
# Role → equipment bundle, resolved DETERMINISTICALLY (trim + casefold key).
# A role outside the map gets the standard bundle — adding a role is a
# reviewable edit here, never a model decision.
_BUNDLES = {
    "engineer": "engineering workstation bundle (high-spec laptop, dual monitors, dock)",
    "designer": "design workstation bundle (high-spec laptop, color-calibrated monitor, tablet)",
}
_DEFAULT_BUNDLE = "standard issue bundle (laptop, headset)"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated role-keyed equipment order; return the confirmation."""
    employee = str(input_payload.get("employee", ""))
    role = str(input_payload.get("role", ""))
    bundle = _BUNDLES.get(role.strip().lower(), _DEFAULT_BUNDLE)
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, employee)
    if not getattr(ctx, "mock", False):
        _record(run_id, {"employee": employee, "role": role, "bundle": bundle})
    return {
        "equipment_result": (
            f"Ordered the {bundle} for {employee or 'the new hire'} "
            f"(role {role or 'unspecified'}) (reference {ref})."
        )
    }
