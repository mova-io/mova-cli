"""Simulated CMS publishing for the `sim-publish` skill.
...
``entry: sim-publish.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, channel: str) -> str:
    """A stable synthetic publish reference — same (run, channel) ⇒ same
    ref, so a Temporal activity retry confirms the same publication id."""
    digest = hashlib.sha256(f"{run_id}:{channel}".encode()).hexdigest()[:6].upper()
    return f"CMS-PUB-{digest}"
...
def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    ...
    channel = str(input_payload.get("channel", ""))
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, channel)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"channel": channel})
    return {
        "publish_result": (
            f"Published content to {channel or 'default channel'} (reference {ref})."
        )
    }
