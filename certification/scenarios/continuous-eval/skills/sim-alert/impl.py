"""Simulated quality alert for the `sim-alert` skill.
...
``entry: sim-alert.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, score: float) -> str:
    """A stable synthetic alert reference — same (run, score) ⇒ same ref, so
    a Temporal activity retry confirms the same alert id."""
    digest = hashlib.sha256(f"{run_id}:{score:.4f}".encode()).hexdigest()[:6].upper()
    return f"EVAL-ALERT-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated quality alert; return the confirmation line."""
    score = float(input_payload["score"])
    issues = str(input_payload["issues"])
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, score)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"score": score, "issues": issues})
    return {
        "alert_result": (
            f"Quality alert {ref} raised: sampled score {score:.2f} is below the 0.6 floor."
        )
    }
