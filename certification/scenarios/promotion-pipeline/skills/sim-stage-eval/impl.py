"""Simulated staging eval for the `sim-stage-eval` skill.
...
``entry: sim-stage-eval.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, change: str) -> str:
    """A stable synthetic eval reference — same (run, change) ⇒ same ref, so
    a Temporal activity retry confirms the same staging-eval record."""
    digest = hashlib.sha256(f"{run_id}:{change}".encode()).hexdigest()[:6].upper()
    return f"EVAL-STG-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated staging eval; return the summary line."""
    change = str(input_payload["change"])
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, change)
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"change": change})
    return {
        "stage_eval_result": (
            f"Staging eval {ref} for '{change}': offline eval suite scored 0.91 overall "
            "(simulated, deterministic) — ready for sign-off."
        )
    }
