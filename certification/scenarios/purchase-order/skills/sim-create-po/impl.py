"""Simulated ERP purchase-order creation for the `sim-create-po` skill.
...
``entry: sim-create-po.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, item: str) -> str:
    """A stable synthetic PO reference — same (run, item) ⇒ same ref, so a
    Temporal activity retry confirms the same purchase-order id."""
    digest = hashlib.sha256(f"{run_id}:{item}".encode()).hexdigest()[:6].upper()
    return f"PO-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated PO creation; return the confirmation line."""
    item = str(input_payload.get("item", ""))
    requester = str(input_payload.get("requester", ""))
    amount = input_payload.get("amount", 0)
    run_id = str(input_payload.get("run_id") or getattr(ctx, "run_id", "") or "")
    ref = _reference(run_id, item)
    if not getattr(ctx, "mock", False):
        _record(run_id, {"item": item, "requester": requester, "amount": amount})
    return {
        "po_result": (
            f"Created purchase order for {item or 'item'} ({amount}) requested by "
            f"{requester or 'requester'} (reference {ref})."
        )
    }
