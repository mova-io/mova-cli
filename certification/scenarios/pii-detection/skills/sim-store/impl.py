"""Simulated DLP clean-store for the `sim-store` skill.
...
``entry: sim-store.impl:run`` in skill.yaml resolves this file via a PEP 420
...
def _reference(run_id: str, source: str) -> str:
    """A stable synthetic storage reference — same (run, source) ⇒ same
    ref, so a Temporal activity retry confirms the same storage id."""
    digest = hashlib.sha256(f"{run_id}:{source}".encode()).hexdigest()[:6].upper()
    return f"DLP-STORE-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated clean-store; return the confirmation line."""
    ...
    if not getattr(ctx, "mock", False):
        _record(run_id, _ACTION, {"source": source})
    return {
        "store_result": (
            f"Stored clean document from {source or 'unknown source'} (reference {ref})."
        )
    }
