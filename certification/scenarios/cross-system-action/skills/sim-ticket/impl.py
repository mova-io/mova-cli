"""Simulated ITSM ticket creation for the `sim-ticket` skill.
...
``entry: sim-ticket.impl:run`` in skill.yaml resolves this file via a
...
def _reference(run_id: str, source: str) -> str:
    ...
    return f"SNOW-TKT-{digest}"


def run(input_payload: dict[str, Any], ctx: Any = None) -> dict[str, Any]:
    """Record the simulated ticket creation; return the confirmation line."""
    ...
    return {
        "ticket_result": (
            f"Opened the tracking ticket for: {action_request or 'the requested action'} "
            f"(reference {ref})."
        )
    }
